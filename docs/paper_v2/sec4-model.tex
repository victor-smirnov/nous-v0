...
it, and the cut is the phenomenon: the tail the agent could not compute is, from inside, the part of
its own causes that it registers as fixed by nothing it can trace, and the Observer's conclusion is
that registration. This is also the point at which the psychophysical problem of
...
the truncation of the tails of these self-decompositions. The truncation is a fact about a finite
...
it. How the composition is realized in a Transformer, where the functions are not modules but
...
carries incrementality at the grain of the token, and could carry it at a finer grain if its frames
could be located between layers, which a criterion not yet formulated would have to decide
