\section{The Synthea model: higher-order computational phenomena}
\label{sec:model}
...
these are among the most predictable. We call the conceptualized recurring displacements
\emph{higher-order computational phenomena} (\hocp). They are the agent's budget appearing to the agent
as content, and only those that pass the criterion of Section~\ref{sec:criterion} qualify.
...
Two claims, neither of which follows from the positions in Section~\ref{sec:antecedents}. First, the
...
result would refute them.

\paragraph{When a phenomenon is a function.}
\label{sec:emergence}
A \hocp\ is not yet a function of consciousness. The agent must apply its reasoning to itself, which
...
because there is nothing further it can represent. The partial sum is the self-model the agent has. The
tail it did not compute is finite, bounded, and inaccessible to the agent, and the tail is what the
Observer registers as the part without causes. So the regress of Section~\ref{sec:legacy} neither
terminates in a homunculus nor runs forever. A budget cuts it, and the cut is the phenomenon.
...
process that produces the report. The Synthea model gives a particular solution: those phenomena are
grounded in the agent's own physics, the finiteness of its budget, through the phenomena of
