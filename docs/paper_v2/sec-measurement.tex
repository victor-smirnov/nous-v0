...
\subsection{Two parts of the solution}
\label{sec:twoparts}
% S0

The solution has a conceptual part and a quantitative part, and they reach different distances.

The conceptual part is this. Once the Observer is treated as curvature (Section~\ref{sec:criterion},
...
substrate's internal states predicts, against the residual it fails to predict. The residual is what did
not make it in.
...
Above the baseline on a component: hyperfunction. Below it: deficit. Absent: a dropped axis. One function
can be all three on different axes at once, and current models almost certainly are.
...
supply its own weights, which S2b returns to. The gain is that every entry in the vector is a number
someone can get wrong.

\subsection{Generalization is not memorization}
...
with errors that depend on the tokenization and the number of digits, and scaling improves it slowly
\citep{nogueira2021}; on compositional tasks they approach the answer by shortcut and break down
...
\subsection{Two measurements and what their disagreement says}
...
(D10), and not in the function. Benchmark present, probe absent: the carrier is at a granularity the
...
This is no longer a conjecture about Transformers. It is what looking inside them has found.
...
recur across models, with adjacent layers inside a stage robust to deletion and to swapping
\citep{lad2024}; the older finding that a language model rediscovers the classical processing pipeline
...
\subsubsection{Depth as time, and what fixes it when it is too short}
\label{sec:depthtime}
% S3c
...
record of effort, because effort is constant, which is exactly the dropped axis that S5 predicts for
givenness.
...
than a fixed schedule, and past a point the models overthink and degrade \citep{kohli2026}. Reasoning can
also be moved off the tokens altogether by feeding the last hidden state back as the next input, which
yields a continuous chain of thought that can keep several candidate next steps in superposition where a
written chain has to commit to one \citep{hao2024}.
...
exactly the kind Section~\ref{sec:hocp} predicts: a systematic displacement produced by the budget,
...
\subsubsection{Two time axes}
% S3d

The objection of Section~\ref{sec:frozen} sees one time axis. Along tokens the model is as the objection
...
\subsection{The Transformer's profile of seeing}
...
\subsection{Predictions carried over}
...
\subsection{The other end of the scale}
\label{sec:zero}
% S7a
...
\subsection{What the argument reaches}
% S8
...
goes to the illusion problem, as D2 has it, and the illusion problem is what
