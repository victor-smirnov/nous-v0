...
relevant to a brain has a compact algorithmic description, and what stops a reconstruction from outside
...
\item \emph{A base functional model, higher-order computational phenomena} (Section~\ref{sec:model}).
  A finite budget displaces reasoning from its unbounded ideal in the same places every time; a
  self-modelling agent carries a compressed model of these displacements. From this the Observer, the
  convergence of the self-model regress, the two faces of one residual, the stack of Observer, Agent
  ...
  side and the self side are one residual, and the phenomena leave a measurable trace.
...
this paper. In passing, the framework gives a particular solution to the psychophysical problem: the
phenomena of the self-report level, freedom, unity, the quale, the gap to other minds, are grounded in
the agent's own physics through the phenomena of Section~\ref{sec:model}
