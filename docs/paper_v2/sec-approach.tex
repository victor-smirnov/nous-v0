...
\subsection{A function is a pattern that recurs, and what recurs is compressible}
\label{sec:function}
% A1, A2
...
Recurrence has an information-theoretic consequence, and the thin definition inherits it. If a pattern is produced
...
say on who is doing the compressing, and the gap between the constants of two substrates is
...
\subsection{The lamp: computational functionalism, pending a proof that would be vitalism}
\label{sec:lamp}
% A3
...
\subsection{The definition: the part of self-reasoning that describes the agent's own reality}
...
\node[box, left=of c] (m) {self-model holds a compressed copy: \hocp};
\node[box, left=of m] (k) {survives every affordable self-model? (the curvature test)};
...
\subsubsection{What we add: one residual, and a measurable trace}
...
different results and are not usually taken to be about the same quantity. Section~\ref{sec:twofaces} shows them as two faces
...
\subsubsection{Only a displacement that survives every self-model counts}
\label{sec:criterion}
% A6c
...
curvature, and they are content.
...
\subsubsection{A phenomenon becomes a function where a self-applying agent is kept asking}
\label{sec:emergence}
% A6d
...
For humans the place is a society of other such agents, which Section~\ref{sec:record} described. For a language
model the asking is in the data and in the use, and Section~\ref{sec:zero} asks how small the asking environment can be.
...
\subsubsection{The Observer survives every refinement of the self-model}
% A8
...
does work. It also tells us what the Observer is not. It is not a belief that
...
the part without causes. So the regress of Section~\ref{sec:legacy} neither terminates in a homunculus nor runs forever. A
budget cuts it, and the cut is the phenomenon. This is the paper's answer to the oldest of the circles in Section~\ref{sec:grammar}, and it costs nothing beyond Section~\ref{sec:physics}.

\subsubsection{In a model, part of what cannot be traced is gone, not hidden}
\label{sec:secondcut}
% A10
...
and it belongs in the profile.

\subsubsection{Both substrates unload state into language}
\label{sec:language}
% A10a
...
at every step. The difference is one of degree and of where the bound sits. One question therefore falls
on both substrates: are new mental states acquired through language, or rebuilt on the spot from what
the receiver already has, the words working as pointers and constraints? The literature on both sides
is in Appendix~\ref{app:language}; the position we expect to hold, and record as open, is the middle
one, that a named state is acquired only when its prerequisites are already present in the receiver.
Nothing later in the paper depends on which way it goes, except the interpretation of the codes of
Section~\ref{sec:codes}.

\subsection{Illusion is the wrong word: the report is an approximation with a definite residual}
...
Approximation in place of illusion is the one amendment we make to illusionism, and it does two things. It restores degrees, which Section~\ref{sec:legacy} noted the denial had removed, and so it makes the comparative question well posed. And it connects to
the difficulty about the reader: what intrapersonal intelligence improves is the quality of the
approximation, so the reader's difficulty in Section~\ref{sec:reader} and the object of study are the same thing seen from
two positions.

\subsubsection{One residual, two faces: freedom and mystery}
\label{sec:twofaces}
% A12
...
what Section~\ref{sec:twoparts} calls the epistemic qualia, and the point of listing them is that they are one quantity in
...
\subsubsection{Responsibility is a requirement the agent places on its own record}
...
\subsubsection{Good has a substrate before it has a theory}
% A15
...
\subsection{Seeing is hard because it needs a point, and the point is the Observer}
\label{sec:seeing}
% A16, A17, A18
...
phenomenal properties toward a subject, and Section~\ref{sec:grammar} is our diagnosis of why that order stalls.
...
\subsubsection{Seeing light has three components: source, manifold, givenness}
\label{sec:light}
% A17
...
evidence about that boundary is the Perky effect: a faint picture projected on a screen while the subject
was asked to imagine the same object was taken for the subject's own imagery, and the substitution went
unnoticed \citep{perky1910}. It is stable from frame to frame in a way the agent's own productions are not, and
...
light is free and is there. This component is the one that a substrate can lack while having the
other two, and Section~\ref{sec:profile-seeing} predicts that current models lack it.
...
as environment, which is the failure mode of source attribution and, by Section~\ref{sec:profile-seeing}, the standing condition of a
model with no sensory channel.

\subsubsection{Locke closed the inverted spectrum by a decision}
% A18
...
\subsection{Reference functions: what the measurement is applied to}
\label{sec:reference}
% A19, A19a
...
\bottomrule
\end{tabular}
\caption{Reference functions (Sections~\ref{sec:observer}, \ref{sec:light},
\ref{sec:need}--\ref{sec:frames}, \ref{sec:responsibility}). Each component is a displacement that
survives a change of self-model (Section~\ref{sec:criterion}); the profile of Section~\ref{sec:profile}
is a vector over the third column.}
\label{tab:functions}
\end{table}
...
is given with its phenomenon and its components (Table~\ref{tab:functions}), and each component is derived as in
Section~\ref{sec:criterion}: a displacement that survives a change of self-model, and not a part of a
machine.

\subsubsection{Two planes: needs and emotions outside, motivations and feelings in the report}
\label{sec:planes}
% A19a
...
\subsubsection{Needs bend the direction of reasoning, emotions bend closure}
\label{sec:need}
% A20
...
\subsubsection{Emotion also writes memory, and the write is registered as reward}
...
One candidate: consolidation is a write to the agent's own long-term model, paid now and repaid later. The
...
event here is a write and not a derivative. We mark this as open, and note that Section~\ref{sec:frozen} and Section~\ref{sec:twoaxes} depend on
the distinction between this office and the other one, not on this particular candidate.

\subsubsection{The Field is small, and its finiteness is felt as the cost of holding}
\label{sec:field}
% A21
...
\subsubsection{Frames: the Observer is re-derived in every frame}
\label{sec:frames}
% A22
...
\subsubsection{Three constructions the account owes}
...
\subsubsection{Responsibility as a function}
% A23
...
\subsection{The Transformer is a forward-chaining rule system over vectors}
...
events, and self-reference is its ordinary mode of operation.

\subsubsection{Functional equivalence licenses the comparison of substrates}
% A24a
...
the same function whatever they are made of. That is what
...
\subsubsection{Prediction as the formalism: the structure is never written down, its effects are measurable}
\label{sec:formalism}
% A24b
...
structure to be named, and Section~\ref{sec:h1h2} said that most of the structure has no name.
...
what has no name cannot be specified by hand, which is Section~\ref{sec:record} again in formal dress. Program induction has
...
\subsubsection{A predictor trained on the record is under pressure to generalize the Observer}
% A24d
...
Something crosses the cut of Section~\ref{sec:cut}, or the phenomenon of Section~\ref{sec:intro} would not hold. What crosses is the encoding of
...
The components of the previous claim are separately supported, and Appendix~\ref{app:codes} collects the
evidence: that a writer's state is in the statistics of a text beyond its content, that a receiver
rebuilds the sender's state to the degree the rebuilding is faithful, that the bridge crosses substrates,
and that internal states are readable and largely linear. One literature has to be read the right way
round. Studies that find episodes where a model's chain of thought does not match the process that
produced its answer \citep{lanham2023,turpin2023} test a local correspondence: this token sequence
against this computation. The correspondence claimed here is statistical, over many episodes. A single
episode may deviate as far as the variance allows, in models as in humans, where the corresponding
result is that people confabulate causes of their own behaviour in the same way \citep{nisbett1977}.
The well-posed test is the correlation stated in Section~\ref{sec:predictions}, with the variance
reported as a measured part of the profile.
