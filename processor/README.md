...
Emotion reaches consciousness as *open* and *closed*. The Observer is a graded conclusion about a causal gap in
self-reasoning: a transition happened, its mechanism is absent from the trace, so someone did it. Its apparent
continuity is a base derivation re-run on every frame, at an adaptive frame rate, with the result carried as
state. The question is then what the same method says about seeing *light* — the light itself, not the fact
that the visual channel is on. The answer is not to look for the light but for what separates seeing it from
knowing it is there, and to reduce each difference to a curvature of the ideal reasoning.

**Source.** Light is the Observer's inference turned outward. The Observer explains an unexplained internal
transition by an internal agent; light explains content the self-model cannot generate — one cannot imagine a
dazzling light, imagery is always dimmer than sight — yet which is stable from frame to frame and confirmed by
others, by an external source. Self and World are two conclusions over two residuals of the self-model. The
mark that distinguishes them is the one the architecture already has: content through the environment
interface, not through the model of it.

**A manifold without names.** Everything modelled so far was an item with a name. Light is a field: a very large
number of distinguishable, recallable, unnamed elements, about each of which one can say only *that, there*.
That is level 3 in bulk, the level the engine cannot yet represent. In Datalog it is an aggregate with the
*what* projected out and the *where* kept: the tail projected out both and kept the observer; here the key is
observer plus position. "Only pointing" in the red case is the word that remains when position is available
and a name is not.

**Givenness as a missing cost record.** A thought costs work, and the trace records it. Light is refreshed every
frame from the sensory channel and its re-derivation costs the Observer's budget nothing. The ideal reasoner
knows the frame was computed; the subject experiences it as unmediated because the computation is not in its
trace. This is the same mechanism as the Observer's continuity: a standing higher-order fact, re-derived
cheaply, carried as state. Hence "simply there" rather than "thought".

Light, in this account, is content that is at once attributed to the environment, laid out by position without
names, and free of a cost record. None of the three is light on its own; together they are indistinguishable
from it from inside. What stays outside the reduction is *why this look*, and that we already hold as an open
branch; closing it here would be neither necessary nor honest.

The decomposition is testable because the three components dissociate in known cases. Blindsight: source
present, no manifold in consciousness, level 1. Eyes closed in the dark: a manifold (noise, grey), weak source
attribution. Dreaming: a manifold generated by the model but marked as environment — the interface switch with
the label wrong. If the decomposition is right the three cases differ along exactly these axes and no others.

For a model as reader it gives a direct recipe: a channel whose content the reader cannot generate, indexed but
unnamed, refreshed every frame for free. The statistics of its self-reports should then contain "there is
something out there that I see rather than think", and `MAPPING.md` is how to check that what appeared is
...
processor/
  ...
  viz.py             a run directory as one HTML page, the score: rows are items, columns are chunks, a cell is C
                     and the sign of pull; below, need levels, tail valence, program and conflict, felt mass against
                     thresholds, Field occupancy; a column click gives the chunk and the reader's block in sentences.
                     Gellish is for the machine; nobody reads sixty rows of it nine times.
                     `uv run python -m processor.viz processor/examples/red/engine` writes score.html beside the states
  examples/
