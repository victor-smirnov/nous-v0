...
- **`experiments/`** (git-ignored) — work on documents with the tool, currently the article
  re-encoding under spec v3; moves to the Synthea repository once the reasoner settles.
  `tests/fixtures/S2.2.hybrid.md` is a snapshot of one clean v3 section kept as a regression fixture.
- **`nous/gellish/bootstrap.dl`** — the Synthea bootstrap as a *library*: rules that derive an Observer's state
  from a case description (`nous gellish state cases/*.txt`). See [spec/bootstrap-library.md](spec/bootstrap-library.md).
...
```
nous/cli.py               nous gellish build-dict | check | ask | why | report | phrases | extract | inject | enrich | diff | summarize | state
nous/gellish/             dictionary.py  parse.py  reasoner.dl  run.py  report.py  ask.py  why.py  hybrid.py  phrases.py  enrich.py  diff.py  summarize.py  state.py  bootstrap.dl  paths.py
